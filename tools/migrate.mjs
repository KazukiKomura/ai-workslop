...
function d1(command){const r=spawnSync('npx',['--no-install','wrangler','d1','execute','handoff-experiment',target,'--json','--command',command],{cwd,encoding:'utf8'});if(r.status!==0){process.stderr.write(r.stderr||r.stdout);process.exit(r.status||1)}const text=r.stdout.slice(r.stdout.indexOf('['));return JSON.parse(text)}
