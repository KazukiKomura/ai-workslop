...
async function run(name,viewport){
 ...
 const consoleErrors=[],failed=[],pageErrors=[];page.on('console',m=>{if(m.type()==='error')consoleErrors.push(m.text().slice(0,200))});page.on('pageerror',e=>pageErrors.push(String(e).slice(0,200)));page.on('response',r=>{if(r.status()>=400&&!r.url().includes('/api/action'))failed.push(r.status()+' '+r.url().slice(0,120))});
 ...
 const consent=await page.locator('.consent-screen').innerText();assert(consent.includes('1 件の案件')&&consent.includes('制限時間は 30 分')&&consent.includes('10〜20 分'),'consent wording');mark('consent');
 ...
 await page.locator('[data-instruction-step="0"] h2').waitFor();const intro=await page.locator('[data-instruction-step="0"]').innerText();assert(intro.includes('1 件の案件を担当')&&intro.includes('回答は注意深く確認します')&&intro.includes('30 分以内'),'intro wording');await shot('02-intro');
 ...
 const disclosed=!!st.disclosureText;assert.equal((await page.locator('.handoff').innerText()).includes('生成AI'),disclosed,'handoff line matches disclosure arm');assert.equal(await page.locator('.disclosure-badge').count(),disclosed?1:0,'badge matches disclosure arm');assert.equal(await page.locator('.notice.disclosure').count(),disclosed?1:0,'notice matches disclosure arm');
