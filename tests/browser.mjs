...
await waitPhase(page,'recall');await fillBlock(page,{memory:2,belief:3});await page.screenshot({path:`test-results/${pfx}recall-${tag}.png`,fullPage:true});await next(page);await waitPhase(page,'attitude');await fillBlock(page,{aias_1:5});await page.getByRole('button',{name:'回答を送信して終了する'}).click();await page.getByRole('heading',{name:'ご参加ありがとうございました。'}).waitFor();await page.waitForFunction(()=>document.getElementById('sync').textContent==='保存済み');
...
const ctx=await browser.newContext();const page=await ctx.newPage();await page.goto(base+'/admin');await page.locator('#key').fill(key);await page.getByRole('button',{name:'ログイン',exact:true}).click();await page.getByRole('heading',{name:'参加と記録の管理'}).waitFor();const downloadPromise=page.waitForEvent('download');await page.getByRole('button',{name:'参加者別CSV'}).click();const file=await downloadPromise;await file.saveAs('test-results/participants.csv');const csvText=await readFile('test-results/participants.csv','utf8');for(const col of ['cognition_1.suff_1','cognition_1.readcheck_correct','post_3.tlx_md','trust_post_2.tr_1','responsibility_2.responsibility_influence','perception_3.perc_6','background.ptt_1','reflection.resp_free','recall.memory','case_2','condition_3','sequence'])assert(csvText.includes(col),col);await page.screenshot({path:'test-results/admin.png',fullPage:true});await ctx.close();
