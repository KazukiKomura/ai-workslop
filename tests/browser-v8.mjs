...
const consentText=await page.locator('.consent-screen').innerText();assert(consentText.includes('1 件の案件'),'consent mentions one case');assert(consentText.includes('制限時間は 30 分'),'time limit 30');assert(consentText.includes('10〜20 分'),'duration text');
...
await page.locator('[data-instruction-step="0"] h2').waitFor();const intro=await page.locator('[data-instruction-step="0"]').innerText();assert(intro.includes('1 件の案件を担当'),'intro one case');assert(intro.includes('回答は注意深く確認します'),'audit notice');assert(intro.includes('30 分以内'),'intro time limit');
