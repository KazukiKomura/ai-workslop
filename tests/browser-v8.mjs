...
await page.locator('[data-instruction-step="0"] h2').waitFor();const intro=await page.locator('[data-instruction-step="0"]').innerText();assert(intro.includes('1 件の案件を担当'),'intro one case');assert(intro.includes('回答は注意深く確認します'),'audit notice');assert(intro.includes('資料 S1〜S3 に照らして確認'),'source-check note in intro');assert(!/分以内/.test(intro),'no time-limit wording in intro');
